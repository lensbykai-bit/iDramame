...
function normalizeStory(story) { return { ...story, placement: 'web', telegram_url: '' }; }
...
function saveStoriesLocal(stories) { fs.writeFileSync(storiesPath, JSON.stringify(stories.map(normalizeStory), null, 2) + '\n'); }
function storyById(id) { return loadStories().find((story) => story.id === id); }
...
function generateKHQR(order) {
  ...
  let info;
  try {
    info = new IndividualInfo(BAKONG_ACCOUNT_ID, khqrData.currency.khr, BAKONG_MERCHANT_NAME, BAKONG_MERCHANT_CITY, optionalData);
  } catch {
    info = new IndividualInfo(BAKONG_ACCOUNT_ID, BAKONG_MERCHANT_NAME, BAKONG_MERCHANT_CITY, optionalData);
  }
...
app.get('/api/meta', (_req, res) => {
  ...
  res.json({ brand: BRAND_NAME, mode: 'web-only', checkout: BAKONG_TEST_MODE || Boolean(BAKONG_ACCOUNT_ID && BAKONG_TOKEN), testMode: BAKONG_TEST_MODE, accountLibrary: true, mediaUploads: Boolean(BOT_TOKEN && TELEGRAM_STORAGE_CHAT_ID) });
});
...
app.post('/api/checkout/khqr', async (req, res) => {
  ...
  if (!story) return res.status(404).json({ error: 'Story not found.' });
  if (!story.full_video_file_id && !story.full_video_url) return res.status(409).json({ error: 'រឿងនេះមិនទាន់មាន Full Movie សម្រាប់ទិញទេ។' });
...
app.post('/api/checkout/verify', async (req, res) => {
  ...
  try {
    if (BAKONG_TEST_MODE && md5.startsWith('TEST-')) {
      return res.json({ paid: true, testMode: true, transactionHash: md5, watchUrl: `/watch.html?token=${encodeURIComponent(createWatchToken({ storyId, orderId: order.id }))}` });
    }
...
app.get('/api/access', (req, res) => {
  ...
  return res.json({ story: { id: story.id, title: story.title }, orderId: payload.orderId, videoUrl: `/api/video/${encodeURIComponent(story.id)}?token=${encodeURIComponent(req.query.token)}` });
});
app.get('/api/video/:id', async (req, res) => {
  const payload = verifyToken(req.query.token);
  if (!validWatchPayload(payload) || payload.storyId !== req.params.id) return res.status(401).send('Unauthorized');
  const story = storyById(req.params.id);
...
app.post('/api/admin/upload', adminAuth, mediaUpload.single('file'), async (req, res) => {
  ...
  if (!['cover', 'trailer', 'full'].includes(kind)) return res.status(400).json({ error: 'Upload type is invalid.' });
  ...
  if (kind !== 'cover' && !req.file.mimetype.startsWith('video/')) return res.status(400).json({ error: 'Trailer/Full Movie ត្រូវជា Video File។' });
  try {
...
app.post('/api/admin/stories', adminAuth, async (req, res) => {
  ...
  if (![coverUrl, trailerUrl, fullUrl].every(validHttpUrl)) return res.status(400).json({ error: 'Media URLs must use http or https.' });
  const stories = loadStories();
  let story;
  if (id) {
    const index = stories.findIndex((item) => item.id === id);
    if (index < 0) return res.status(404).json({ error: 'Story not found.' });
    story = normalizeStory({ ...stories[index], title, preview, price_khr: price, cover_file_id: coverFileId, preview_video_file_id: trailerFileId, full_video_file_id: fullFileId, cover_url: coverUrl, preview_video_url: trailerUrl, full_video_url: fullUrl, updated_at: new Date().toISOString() });
    stories[index] = story;
  } else {
    story = normalizeStory({ id: slugify(title), title, preview, price_khr: price, cover_file_id: coverFileId, preview_video_file_id: trailerFileId, full_video_file_id: fullFileId, cover_url: coverUrl, preview_video_url: trailerUrl, full_video_url: fullUrl, created_at: new Date().toISOString() });
    stories.unshift(story);
  }
  try {
    const result = await persistStories(stories, `${id ? 'Update' : 'Add'} web story: ${title}`);
...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'idramaai', mode: 'web-only', brand: BRAND_NAME, checkout: BAKONG_TEST_MODE || Boolean(BAKONG_ACCOUNT_ID && BAKONG_TOKEN), bakongTestMode: BAKONG_TEST_MODE, accountLibrary: true, mediaUploads: Boolean(BOT_TOKEN && TELEGRAM_STORAGE_CHAT_ID) });
});
...
app.use((err, _req, res, _next) => {
  if (err instanceof multer.MulterError && err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: 'File ធំពេកសម្រាប់ Upload តាម Web Admin។ សូមប្រើ File តូចជាង 45 MB។' });
