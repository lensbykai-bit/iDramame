...
const SESSION_SECRET = process.env.STREAM_SESSION_SECRET || ACCESS_TOKEN_SECRET || crypto.createHash('sha256').update(`idrama-stream-v2|${ADMIN_PASSWORD}|${process.cwd()}`).digest('hex');
const SESSION_KEY = crypto.createHash('sha256').update(`idrama-stream-session-v2|${SESSION_SECRET}`).digest();
...
function proxyStream(req, res, targetPath, protectedContent = false, onDone = null) {
  ...
  upstreamReq.on('error', error => {
    console.error('[security-proxy-v2]', error.message);
    onDone?.();
...
async function handleAccess(req, res, url) {
  ...
  try {
    ...
    console.error('[secure-access-v2]', error.message);
...
function handleSecureVideo(req, res, storyId) {
  ...
  return proxyStream(req, res, `/api/video/${encodeURIComponent(storyId)}?token=${encodeURIComponent(session.token)}`, true, done);
}
...
child.on('exit', (code, signal) => {
  console.error(`[security-gateway-v2] Internal app stopped (code=${code}, signal=${signal || 'none'}).`);
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/health') {
    res.statusCode = 200;
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    return res.end(JSON.stringify({ ok: true, securityGateway: 'v2-account-bound' }));
  }
...
server.listen(OUTER_PORT, '0.0.0.0', () => {
  console.log(`[security-gateway-v2] Public :${OUTER_PORT} -> internal ${INNER_HOST}:${INNER_PORT}`);
  console.log(`[security-gateway-v2] Account-bound playback enabled; session ${Math.round(STREAM_SESSION_TTL_MS / 60000)} minutes`);
});
function shutdown(signal) {
  console.log(`[security-gateway-v2] ${signal} received.`);
